...
cloneSamplesRepo() {
    ...
    if [ -d test-network ]; then
        ...
        proxychains4 git clone -b main https://github.com/hyperledger/fabric-samples.git && cd fabric-samples
    fi
...
download() {
    ...
    proxychains4 curl -L --retry 5 --retry-delay 3 "${URL}" | tar xz || rc=$?
    if [ -n "$rc" ]; then
